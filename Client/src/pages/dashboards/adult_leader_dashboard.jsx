...
function Adult_Leader_Dashboard() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/adult_leader_dashboard/news/${user.id}`)
      ...
      {unpaidCamps.length > 0 ? 
        unpaidCamps.map((camp) => (
          <div key={camp.camp_id} className="mb-4">
            ...
            <p>Payment_id: {camp.payment_id}</p>
            <p>Registration_id: {camp.registration_id}</p>
            <button
              className="mt-2 px-4 py-2 bg-blue-500 text-white rounded shadow hover:bg-blue-600 focus:outline-none"
              onClick={() => navigate('/adult_leader_functions/campers_card_payment', { state: { camp_name: camp.camp_name, paymentId: camp.payment_id, registration_id: camp.registration_id, finalPrice: camp.amount, user_id: user.id } })}              >
              Manage Payment
            </button>
          </div>
        )) : 
